...
class NewFoodTableViewCell: UITableViewCell {
    ...
    var doubleTapRecongnizer: UITapGestureRecognizer? = nil
...
//        self.dislikeButton.clipsToBounds = true
        ...
        self.doubleTapRecongnizer = UITapGestureRecognizer(target: self, action: #selector(self.isLiked(_:)))
        self.doubleTapRecongnizer!.numberOfTapsRequired = 2
        self.addGestureRecognizer(self.doubleTapRecongnizer!)
    ...
    @objc func isLiked(_ button: UIButton){
        if !self.isLikedOrDisliked{
            AppIOManager.shared.foodDataAction(food_id: self.data!._id, method: "plus"){isPassed in
                let textLabel = UILabel(frame: CGRect(x: -40, y: 10, width: 60, height: 40))
                textLabel.text = "like +1"
                ...
                UIView.animate(withDuration: 0.4){
                    textLabel.x = 10
                    self.visualEffectView.x += 20 + 30
                    self.visualEffectView.width -= 20 + 30
                }
